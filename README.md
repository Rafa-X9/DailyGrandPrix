...
This project used CSV to store data because, when I started making this series, it was the only "database" I knew how to use. Later I changed to JSON to make handling the database easier, as C# has a `System.Text.Json.JsonSerializer` class that automatically parses JSON to object and vice-versa.
## Functionality
...
**WARNING**: it's not recommended to run this project in an IDE's or a code editor's built-in terminal. This project uses .NET's `Console.Clear()` command to clear the console, which might not be supported in those terminals. From my tests, Visual Studio Code's terminal did not support them.
...
21. Simulate race: asks for information to create a new race and the number of simulations. Then it makes that amount of simulations in order to eventually find the optimal strategy. In each simulation, it randomly chooses a strategy for the driver. In my tests, I generated 2,500,000 simulations to achieve a strategy to finish a race in 10 moves.
22. Close program: closes the program.

## Race
### Steps
I have made a simple calculator to easily calculate the amount of steps based on the given variables, [you can find it here](https://rafa-x9.github.io/DailyGPcalc/).

DailyGrandPrix's races are based around **steps**. All tracks are divided in a certain amount of steps. The amount of steps each driver drives at once is calculated based on their tyre compounds, tyre wear, fuel amount, gap to the car ahead, and rain.
...
There are four tyre compounds: softs, mediums, hards, and intermediates. Softs have a drag of 1, therefore are faster, however they last shorter. Hards have a drag of 3, so are slower, but last longer. Mediums have a drag of 2 and are the midterm between speed and durability. Intermediates are tyres intended for driving under the rain, and are very slow and degrade fast if used on a dry track. All tyres start with a wear of 100. 0 is the minimum value for tyre wear.

At the start of the race, all drivers must decide an amount of 0 to 100 to be their starting fuel amount. Less fuel causes the car to be faster, but risks running out of fuel before the race finishes, which causes a driver to retire from the race. More fuel makes the car slower, but is safer to prevent retirements.

Because of slipstream, cars get faster when they are behind another car. The gap ahead is the amount of steps in front the car ahead is. The driver has slipstream added to their speed when there is a car 20 steps or less ahead.

The driver may also choose to push harder or conserve. When conserving, soft tyres' wear reduce by 20; mediums, by 12; hards, by 7; and intermediates, by 15 or 30, depending on rain level; fuel reduces by 5. When pushing, soft tyres' wear reduce by 40; mediums, by 24; hards, by 14; and intermediates, by 30; fuel reduces by 10. The driver can also pitstop and change to a new tyre compound with 100 life. When changing tyres, the new tyres are cold, and therefore the driver's next move cannot be push.
...
First, BaseSteps is calculated:

$BaseSteps = (2.5 + (12.5 * (CompFactor * LifeFactor * (0.6 + (0.4 * FuelFactor)))) * Multiplier) * (1 + (0.15 * Slipstream))$
...
If intermediate tyres, a $Divisor$ is calculated, with $Increase$ being equal to 1.3:

$Divisor = 0.0104 * (RainFactor - 25)²  + Increase$

$Steps = BaseSteps / Divisor$

### Classes
There are four classes drivers can choose from before the start of the race. Each class gives a certain advantage. Drivers can't change class during the race. The classes are:

Oscar Piastri: when not pushing, slick tyres degrade less (softs: 14, mediums: 10, hards: 5).
...
Lance Stroll: in the steps calculation, $Increase$ is always 1 rather than 1.3.
